...
2. **Georeference** — you choose the drawing's source CRS (SIRGAS 2000, WGS 84, SAD69 or Córrego Alegre UTM zones, or a custom proj4 string). A DWG has no reliable CRS field, but it often *says* its CRS somewhere — a title block reading `UTM - SIRGAS-2000 - MC 33° W - FUSO 25 SUL`, or a GEODATA object. The converter surfaces those declarations and the app reads them: when both datum and zone are stated, the CRS is prefilled and the quoted text is shown, so you can see exactly what the choice rests on. Anything weaker (only a zone, only coordinate magnitudes) is offered as a suggestion. The app reprojects every coordinate to WGS 84 in the browser with [proj4js](http://proj4js.org/).
3. **Map** — the reprojected features are drawn on MapLibre over a free basemap (**streets** from [OpenFreeMap](https://openfreemap.org/) — no API key — or **Esri satellite imagery**), styled with each entity's own CAD **colour** and **line weight**, with **TEXT/MTEXT rendered as rotated labels**. A floating **Layers menu** on the map lets you toggle each imported DWG layer (and the text labels), and the view fits the drawing's bounds. Click a feature to inspect its CAD properties.
